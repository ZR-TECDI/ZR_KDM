...
def register(name, ttf_fname, fontd_fname):
    ...
    """
    with open(fontd_fname, 'r', encoding='UTF-8') as f:
...
def create_fontdict_file(css_fname, output_fname):
    ...
    """
    with open(css_fname, 'r', encoding='UTF-8') as f:
        data = f.read()
        res = _parse(data)
        with open(output_fname, 'w', encoding='UTF-8') as o:
            o.write(json.dumps(res, sort_keys=True, indent=4))
        return res
